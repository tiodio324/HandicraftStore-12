...
export const LoginModal = observer(() => {
  ...
  const [selectedRole, setSelectedRole] = useState<LoginRole>('seller');
  ...
  const handleClose = () => { closeLoginModal(); setPassword(''); setSelectedRole('seller'); };

  return (
    <Modal isOpen={loginModalOpen} onClose={handleClose} title="Вход в систему" size="sm">
      <form onSubmit={handleSubmit} className={styles.form}>
        <div className={styles.roleSelector}>
          <button type="button" className={`${styles.roleButton} ${selectedRole === 'seller' ? styles.active : ''}`} onClick={() => setSelectedRole('seller')}>
