...
const AUTH_STORAGE_KEY = 'handicraft_store_auth';
const SESSION_EXPIRY_KEY = 'handicraft_store_session_expiry';
const SESSION_DURATION = 24 * 60 * 60 * 1000;
const AUTH_CREDENTIALS: Record<Exclude<UserRole, 'viewer'>, string> = { seller: 'seller2026-craft', admin: 'admin2026-craft' };
...
export class AuthStore {
  ...
  get isSeller(): boolean { return this._user.role === 'seller' || this._user.role === 'admin'; }
  ...
  canManageProducts = (): boolean => this.permissions.canManageProducts;
  canManageOrders = (): boolean => this.permissions.canManageOrders;
  canManageCategories = (): boolean => this.permissions.canManageCategories;
  canAccessAdmin = (): boolean => this.permissions.canAccessAdmin;

  hasRole = (r: UserRole): boolean => ({ viewer: 0, seller: 1, admin: 2 }[this._user.role] >= { viewer: 0, seller: 1, admin: 2 }[r]);
